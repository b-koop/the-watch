...
export default function (pi: ExtensionAPI) {
	...
	pi.on("agent_start", (_event, ctx) => {
		...
		if (currentStatus) publishStatus(ctx);
	});
	...
	pi.on("session_start", (_event, ctx) => {
		publishStatus(ctx);
		if (!statusTimer) {
			statusTimer = setInterval(() => publishStatus(ctx), 30_000);
		}
	});
